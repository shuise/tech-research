(function($){
    ...
    var tpl = {
        ...
        thead : '<tr><th rowspan=2>浏览器{th1}<tr>{th2}'
    }
    var global = {
        dataUrl :  "brclick.json",
    ...
    if(/\?(\S+)/.test(location.href.replace(/#.*/, ""))){
        ...
        global.dataUrl = "brclick." + global.name + ".json";
    ...
    function drawChart(name){
        ...
        if(name == 'button'){
            ...
            global.canvas.xyChart([{
                ...
                tips : "{y}次访问\n" + (firstName[name] ? firstName[name] + "：{ys.0.percent}%" : ""),
                topLabel : "{percent}%",
