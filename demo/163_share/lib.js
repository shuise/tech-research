var HE = {
	...
	jsonStringify : function(json){
		...
		switch (json.constructor) {
			...
			case Array:
				return '[' + json.map(HE.jsonStringify).join(',') + ']';
			case Object:
				var lxStr = [];
				for (var property in json) 
					if (typeof(json[property]) != "function") {
						lxStr.push(HE.jsonStringify(property) + ':' + HE.jsonStringify(json[property]));
					}
				return '{' + lxStr.join(',') + '}';
			case Number:
				if (isFinite(json)) {
	...
	setHash : function(name,value){
		...
		hash = hash.replace("#!","?");
		location.hash = url.split("#!")[0] + HE.setPara(hash,name,value).replace("?","#!");
	},
	...
	excuteFunc : function(funcName,argument){
		...
		if(HE._typeof(func) == 'function'){
			func.apply(null,argument);		
		}else{
			alert("function is not exist.");
		}	
	},
	_typeof : function(variable){
		var type = Object.prototype.toString.apply(variable);
