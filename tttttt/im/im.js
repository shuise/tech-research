; (function (RCS) {
	...
	var send = function () {
		...
		if (message) {
			...
			sendMessage(new RongIMLib.TextMessage({ content: message, user: RCS.config.user, extra: "附加信息" }));
			inputMsg.value = '';
			inputMsg.focus();
		}
	}
	...
	var sendMessage = function (msg, callback) {
		var targetId = conversation.id || RCS.config.groupId; // 目标 Id
		// alert(targetId)
	...
	var updateMessage = function (message) {
		// console.log("messagemessagemessagemessagemessagemessage")
		// console.log(message)
		...
		// console.log(render(templates.imMessageTemplate, newMessage))
	...
	var openConversation = function (conversation) {
		...
		var callbacks = function (list, hasMsg) {
			...
			data.targetName = RCS.config.title;
			data.terminal = terminal;
			// alert($(".rcs-chat-wrapper")[0].className)
	...
	var messageItem = {
		file: function (file) {
			...
			// 创建文件消息
			var sender = RCS.config.user;
			return new RongIMLib.FileMessage({ name: file.name, size: file.size, type: type, fileUrl: file.downloadUrl, user: sender });
		},
		image: function (image) {
			var sender = RCS.config.user;
			return new RongIMLib.ImageMessage({ content: image.thumbnail, imageUri: image.downloadUrl, user: sender });
		}
	};
	...
	var sdkInit = function (params, callbacks) {
		...
		var messageName = 'MuteMessage';
		var objectName = 'SY:Mute';
		RongIMClient.registerMessageType(messageName, objectName, mesasgeTag, props);

		messageName = 'UnmuteMessage';
		objectName = 'SY:Unmute';
		RongIMClient.registerMessageType(messageName, objectName, mesasgeTag, props);
		...
		RongIMClient.setOnReceiveMessageListener({
			...
			onReceived: function (message) {
				...
				if (message.conversationType == conversationType) {
	...
	//im组件初始化
	var init = function (config, callback, received) {
		RCS.config = config;

		// RCS.config.user.portrait = RCS.config.user.avatar;

		conversation.id = config.groupId;
		config.isIM = true;
		var callbacks = {
			getInstance: function (instance) {
				...
				createButton(config);
				showCommon();
				// openConversation({id:"15711215206",mcount:10})
	...
	RCS.openConversation = openConversation;
	RCS.updateMessage = updateMessage;
})(RCS);