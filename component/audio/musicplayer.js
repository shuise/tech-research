function musicPlayer(obj,data,texts,temp){
	...
	var copy = NE.json.copy;					
	var texts = texts || {};
		texts = copy(texts,textsPadding);
	var temp = temp || {};	
		temp = copy(temp,tempPadding);
	this.obj = obj;
	this.temp = temp;	
	this.data = data;
	this.texts = texts;
...
musicPlayer.prototype.createUI = function(){
	...
	var substitute = NE.template.replace;
	var obj = this.obj;
	var temp = this.temp;
	var data = this.data;
	var texts = this.texts;
		...
		if(this.geter('audio')){
			...
			texts.list = substitute(temp.list,data);
		}	
		obj.innerHTML = substitute(temp.main,texts);
		addClass(obj,'music_player');	
...
musicPlayer.prototype.getObjs = function(){
	...
	var $ = NE.$;
	return {
		play : $('span.mp_play',obj)[0],
		prew : $('span.mp_prew',obj)[0],
		next : $('span.mp_next',obj)[0],
		single : $('span.mp_single',obj)[0],
		circle : $('span.mp_circle',obj)[0],
		add : $('span.mp_add',obj)[0],
		clear : $('span.mp_clear',obj)[0],
		
		title : $('div.mp_title',obj)[0],
		prog : $('div.mp_prog',obj)[0],
		time : $('div.mp_time',obj)[0],
		volume : $('span.mp_volume_slide',obj)[0],
		muted : $('span.mp_muted',obj)[0],
		
		audio : $('section.mp_audio audio',obj)[0],
		list : $('div.mp_list ul',obj)[0],
		items : $('div.mp_list li a.mp_item',obj),
		dels : $('div.mp_list li a.mp_del',obj)
	}	
}
musicPlayer.prototype.oprator = function(){
	...
	array.each(dels,function(index,item){
		item.onclick = function(){
...
musicPlayer.prototype.modeTrans = function(obj,mode){
	var objs = this.getObjs();
	this.mode = mode;		
	NE.classList.remove(objs.single,'now');
	NE.classList.remove(objs.circle,'now');
	NE.classList.add(obj,'now');
}
musicPlayer.prototype.volumSlide = function(obj,event,player){
	var v =  getOffsetX(obj,event);
	var w = parseInt(obj.clientWidth);
	player.volume = v;
	NE.$('em',obj)[0].style.width = v*w + "px";
}
musicPlayer.prototype.muted = function(obj,player){
	if (player.muted){
		player.muted = false;
		NE.classList.remove(obj,'gray');			
	}else{
		player.muted = true;
		NE.classList.add(obj,'gray');
	}
}	
musicPlayer.prototype.progBar = function(){
	...
	var guid = NE.string.guid;
	var $ = NE.$;

	if($('em',progBar).length == 0){
		var id = guid();
		var	em = objMock(id,progBar,'em');
			em.style.width = '1px';
	}
	var prog_grade = NE.$('em',progBar)[0];	
	...
	function objMock(obj,objP,tagname,_class){
		var isStr = typeof(obj) == 'string';
		var id = isStr ? obj : guid();
		var obj = isStr ? $('#' + obj) : obj;
		var objP = objP || $('body')[0];
		if(!obj){
...
musicPlayer.prototype.add = function(btn){
	...
	pop1.init();
	var pop = pop1.nodes.popBox;
	var btn = NE.$('a',pop)[0];
	var sub = NE.$('input.submit',pop)[0];
	sub.onclick = function(){
		var obj = NE.$('input.text',pop)[0];
		if (isURL(obj.value)){
			var url = obj.value;
	...
	function addSong(url,singer,name){
		var name = name || '';
		_this.len += 1;
		list.innerHTML += substitute(_this.temp.list,{url:url,singer:singer,name:name});
