     ...
     *  @tips       为包含 .js_share 类名的元素进行点击事件绑定，使用该元素的属性 data-type 的值 或 类名 .js_share_{type} 的后缀进行分享渠道区分，该版本不计分享数
    ...
    (function() {
        ...
        var shareParam = {},
            js_share_config = window.js_share_config || {};
        ...
        if (!window.js_share_unbind && NE(".js_share_qrcode").length && typeof QRCode === "function") {
            var qrcode = NE(".js_share_qrcode"),
                js_share_qrcode, qr_size = parseInt(js_share_config.qr_size, 10) || 120;
            ...
            qrurl = qrurl.split("#")[0] += "#sns_weixin"; 
        ...
        function shareto(type, link, title, description, pic, origin, sign, apk, ralateUid) {
            ...
            // 设置默认标记为#，分享渠道标记及分享次数均显示于此标记之后。可根据需要修改为?
            var anchor = "#",
                _anchor = sign || js_share_config.anchor || "#";
            if (_anchor === "?") {
                anchor = "?";
            }
            ...
            case "weibo":
            case "sinawb":
                // url = params.set(url, "sns_weibo", "ntes_share", anchor);
                // url = params.set(url, "ntes_share_count", ntes_share_count, anchor);
                url = url.split("#")[0] += "#sns_weibo";
